...
export default class RadarCandidate extends Model {
  ...
  @Column({ type: DataType.DECIMAL(10, 2), allowNull: true }) declare EstimatedSalePrice?: number;
  ...
  @Column({ type: DataType.INTEGER, allowNull: false, defaultValue: 0 }) declare ConfidenceScore: number;
  @Column({ type: DataType.STRING, allowNull: false, defaultValue: "DISCOVERED" }) declare Status: string;
  @Column({ type: DataType.JSONB, allowNull: true }) declare Evidence?: object;
}
