...
export async function discoverCandidate(req: Request, res: Response) {
  ...
  const row = await RadarCandidate.create({
    Title: d.title,
    Season: d.season || null,
    EstimatedSalePrice: Number(d.estimatedSalePrice),
    EstimatedMarketplaceFee: Number(d.estimatedMarketplaceFee || 0),
    EstimatedShippingCost: Number(d.estimatedShippingCost || 0),
    PackagingCost: Number(d.packagingCost || 0),
    DemandScore: Number(d.demandScore || 50),
    CompetitionScore: Number(d.competitionScore || 50),
    SeasonalScore: Number(d.seasonalScore || 50),
    TrendScore: Number(d.trendScore || 50),
    MarketScore: score,
    ConfidenceScore: 100,
    Status: getCandidateStatus(score, 100, true),
    Evidence: { source: "MANUAL", dataClassification: "USER_PROVIDED" },
  });
  res.status(201).json(row);
}

export async function getRadarCandidates(_req: Request, res: Response) {
  res.json(await RadarCandidate.findAll({ where: { Status: { [Op.ne]: "ARCHIVED" } }, order: [["MarketScore", "DESC"], ["ConfidenceScore", "DESC"]], limit: 20 }));
}
