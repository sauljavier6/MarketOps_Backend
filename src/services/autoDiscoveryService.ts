  ...
  try {
    ...
    for (let index = 0; index < selected.length; index += 1) {
      ...
      try {
        ...
        const targets = products.length ? products : [trend.keyword];
        concreteProductsFound += products.length;

        for (const title of targets.slice(0, 3)) {
          ...
          try {
            ...
            if (Number(result.research.winnerPriceSampleCount || 0) > 0 || Number(result.research.bravePriceSampleCount || 0) > 0) withMarketPrice += 1;
            else withoutPrice += 1;
            results.push({ ...result.research, sourceTrend: trend.keyword, concreteProduct: products.length > 0 });
          } catch (error: any) {
    ...
    const summary = { trendsFound: trends.length, trendsAnalyzed: selected.length, concreteProductsFound, uniqueProductsResearched: researchedTitles.size, catalogMatches, withoutCatalogMatch: Math.max(0, researchedTitles.size - catalogMatches), withMarketPrice, withoutMarketPrice: withoutPrice, candidatesCreated: created, failed: results.filter((row) => row.status === "FAILED").length, results };
